...
class ShoppinglistStepDefs extends ScalaDsl with EN {
    ...
    And("""^I ask it to get the shopping list options for "([^"]*)"$"""){ (url:String) =>
        recipeUrl = Some(url)
        res = proxy.shoppingListOptionsQuery(ShoppingListOptionsRequest(sessionId.get, url, None, None))
    }

    And("""^I change the option for "([^"]*)" to "([^"]*)"$"""){ (ingredient:String, optionNew:String) =>
    ...
    And("""^I add the recipe to a shopping list at "([^"]*)"$"""){ (store:String) =>
        proxy.addToShoppingListQuery(AddToShoppingListRequest(sessionId.get, recipeUrl.get, None, Some(store), toChange))
    }

    Then("""^shopping list should include "([^"]*)"$"""){ (storeItemName:String) =>
